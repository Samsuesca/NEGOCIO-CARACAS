...
class Empaq(Pestana):
    ...
    def insertData(self): 
        id_prenda, ok = QInputDialog.getInt(self,'Insertar lote en Empaque','Insertar el ID de la prenda:')
        quantity, ok1 = QInputDialog.getDouble(self, 'Insertar lote en Empaque', 'Ingresa la cantidad:')
    ...
    def editData(self):
        ...
        row_id, ok = QInputDialog.getInt(self, 'Editar Lote en Empaque', 'Ingresa el ID de la fila que deseas editar:')

        # Si el usuario hizo clic en el botón "OK" y proporcionó un ID válido, continuar con la edición
        if ok and row_id:
            row = get_id(self.table_name,row_id,ip=self.ip)
            if row is None:
                ...
                id_prenda, ok = QInputDialog.getInt(self,'Editar lote en Empaque','Ingresa el nuevo ID de la prenda:')
                quantity, ok1 = QInputDialog.getDouble(self, 'Editar lote en Empaque', 'Ingresa la nueva cantidad:')
                uptade_date(self,ok,id_prenda,row_id,'id_prenda',ip=self.ip)
                uptade_date(self,ok1,quantity,row_id,'cantidad',ip=self.ip)
             
    def deleteData(self):
        # Obtener el ID de la fila que se desea eliminar
        row_id, ok = QInputDialog.getInt(self, 'Eliminar Lote de Empaque', 'Ingresa el ID que deseas eliminar:')
        if ok and row_id:
